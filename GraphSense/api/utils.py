...
def get_entity_details(currency, entity):
	...
	with ApiClient(configuration) as api_client:
		...
		try:
			...
			error = ("Exception when calling list_address_tags_by_entity: " + str (e))
...
def create_entity_with_details(json_result,currency,query_type,response): # Query_type is one of : "known_entities", "tags", "details", "cluster"
	...
	if query_type == "details":
		if not ('address' in json_result[0]): # This means this is a cluster not a cryptocurrency address
			set_type = "maltego.CryptocurrencyWallet"
			cluster_ID = json_result[0]['entity'] #the Cluster ID is known as "entity" in GraphSense API json result
			entity = response.addEntity(set_type, cluster_ID)
			json_result = get_entity_details(currency,json_result[0]['entity'])
			if json_result[2] :
				error = "Got an error checking the cluster details: " + json_result[2]
				return "", error #end this and report the error
			#json_result = json_result[0]
			entity.addProperty("num_addresses", "Number of addresses", value=json_result[0]['no_addresses'])
		else:
		...
		entity.addOverlay("", OverlayPosition.NORTH_WEST, OverlayType.TEXT)# first we reset the overlay
		if (json_result[1]['address_tags']) or ('best_address_tag' in json_result[1]):
			tags = json_result[1]
		...
		balance_value = json_result[0]['balance']['value']

		total_received_value = json_result[0]['total_received']['value']
		total_sent_value = json_result[0]['total_spent']['value']
		balance_fiat_value = json_result[0]['balance']['fiat_values'][0]['value']
		balance_fiat_currency = json_result[0]['balance']['fiat_values'][0]['code']
		num_transactions = json_result[0]['no_incoming_txs']+json_result[0]['no_outgoing_txs']
		cluster_ID = json_result[0]['entity']
		total_throughput = total_received_value + total_sent_value
		last_tx_datetime = datetime.fromtimestamp(json_result[0]['last_tx']['timestamp'])
		first_tx_datetime = datetime.fromtimestamp(json_result[0]['first_tx']['timestamp'])
		...
		entity.addProperty("num_in_transactions","Number of incoming transactions", value=json_result[0]['no_incoming_txs'])
		entity.addProperty("num_out_transactions","Number of outgoing transactions", value=json_result[0]['no_outgoing_txs'])
	...
	if query_type == "cluster":
		...
		entity.addOverlay("", OverlayPosition.NORTH_WEST, OverlayType.TEXT)# first we reset the overlay
		if (json_result1['address_tags']) or('tags' in json_result1) or ('best_address_tag' in json_result1):
			entity.addOverlay('Businessman', OverlayPosition.NORTH_WEST, OverlayType.IMAGE)
	...
	if query_type == "tags":
		...
		json_result = get_address_details(currency, json_result[0]['address']) # We query the entity to get the "Address" and the "Entity" (cluster) tags if any.
			...
			for tag in tags:
				if "label" in tag:
					if tag["label"]:
						...
						entity=response.addEntity("Cryptocurrency", "Undisclosed, contact: " + tag["tagpack_creator"])
						entity.setLinkLabel("To tags [GraphSense] (" + tag["currency"] + ")")
						entity.setType("maltego.CryptocurrencyOwner")
				...
				if "tagpack_creator" in tag:
					entity.addProperty("tagpack_creator", "Tagpack Creator", "strict", value=tag["tagpack_creator"])
				if "tagpack_title" in tag:
					entity.addProperty("tagpack_title", "Tagpack Title", "loose", value=tag["tagpack_title"])
				if "abuse" in tag:
					entity.addProperty("Abuse_type", "Abuse type", "loose", value=tag["abuse"])
				if "category" in tag:
					entity.addProperty("Category", "Category", "loose", value=tag["category"])
				if "confidence_level" in tag:
					entity.addProperty("Confidence_level", "Confidence Level", "loose", value=tag["confidence_level"])
	...
	if query_type == "entity_tags": #(that's the tags at cluster level)
		tags = []
		set_type = "maltego.CryptocurrencyOwner"
		json_result = get_entity_details(currency, json_result[0]['entity']) # "entity" = Entity Wallet which is the GraphSense Cluster ID
		if (json_result[2]):
			error = "Got an error getting the cluster tags: " + json_result[2] + ". Entity: " + str(json_result[0]['entity'])
			return "", error
		else:
			json_result = json_result[1]
		if ('tags' in json_result) or ('best_address_tag' in json_result):
